# check-component

AAAC: `/check-component <domain> "<intent>"`

**Layer:** code  
**Validate or inspect** a **component** — UI component, view, or composable unit.

## Dispatch

1. [.cursor/aaac/dispatch.md](../aaac/dispatch.md)
2. [.cursor/aaac/graph.yaml](../aaac/graph.yaml) — **`check-component`**
3. [skills/shared/verbs/check/orchestrator/SKILL.md](../skills/shared/verbs/check/orchestrator/SKILL.md) (object: `component`)

Domain slug recommended.

