...
11. **execute** — [execution](../../execution/SKILL.md) + object_skills + module-authoring when listed — **prod/source only** (hooks deny test paths)
12. **test_execute** — [test-authoring](../../test-authoring/SKILL.md) — **1** test-author Task agent; **only** phase that may Write test files; `test_plan.files_written` required when plan lists `tests_to_add`
